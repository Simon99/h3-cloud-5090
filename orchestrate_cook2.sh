...
BINS=("0,3,4" "5,7,8" "9,10")
DEADLINE_MIN=95; T0=$(date +%s)
...
base=[x for x in o if x.get('dph_total',9)<0.7 and (x.get('inet_down_cost') or 0)<=0.005 and float(x.get('cuda_max_good') or 0)>=13.0]
...
while read -r OF DPH DBW && [ $NPOD -lt 3 ]; do
