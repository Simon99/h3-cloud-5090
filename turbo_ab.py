...
W, H, FR, STEPS = 1312, 736, 141, 20  # 預設;可用 --w/--h 覆寫
...
def build(names, size_mode, seed, prefix, turbo=False, steps=None, te=None, w=None, h=None):
    PROMPT_HOLDER = PROMPT
    ST = steps or STEPS
    g = {
        "L_model": {"class_type": "UNETLoader", "inputs": {"unet_name": UNET, "weight_dtype": "default"}},
        "L_clip": {"class_type": "CLIPLoader", "inputs": {"clip_name": (te or CLIP), "type": "minimax"}},
        ...
        "R2V": {"class_type": "MiniMaxH3ReferenceToVideo",
                "inputs": {"clip": ["L_clip", 0], "vae": ["L_vvae", 0], "audio_vae": ["L_avae", 0],
                           "prompt": PROMPT_HOLDER, "width": (w or W), "height": (h or H), "length": FR,
        ...
        "sched": {"class_type": "BasicScheduler",
                  "inputs": {"model": ["L_model", 0], "scheduler": "simple", "steps": ST, "denoise": 1.0}},
    ...
    if turbo:
        # 記憶教訓:內建 LoraLoader 對此 LoRA 會靜默失敗(518 keys),必須用專用節點
        g["TL"] = {"class_type": "MiniMaxH3TurboLoRA",
                   "inputs": {"model": ["L_model", 0],
                              "lora_name": "minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors",
                              "strength": 1.0, "low_vram": False}}
        g["guider"]["inputs"]["model"] = ["TL", 0]
        g["sched"]["inputs"]["model"] = ["TL", 0]
        g["sampler"] = {"class_type": "MiniMaxH3TurboSampler", "inputs": {}}
    return g
...
def main():
    ...
    wf = build(names, o["size"], int(o["seed"]), out,
               turbo=(o["turbo"] == "1"), steps=(int(o["steps"]) if o["steps"] else None),
               te=("qwen3vl_32b_heretic_minimax_h3_nvfp4.safetensors" if o["te"] == "heretic" else None),
               w=(int(o["w"]) if o["w"] else None), h=(int(o["h"]) if o["h"] else None))
    print(f"  turbo={o['turbo']} steps={o['steps'] or STEPS}")
